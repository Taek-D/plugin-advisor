...
Claude Code 플러그인 추천 + 조합 분석 웹 서비스. 프로젝트 분석(/advisor)으로 맞춤 플러그인 추천, 조합 분석(/optimizer)으로 현재 설치된 MCP 서버/Plugin 점수화 + 타입별 보완/대체 추천. 42개 MCP + 9개 Plugin 검증 DB, 한/영 다국어 지원, 어드민 패널 포함.
...
## Current Milestone: v1.3 DB 확장

**Goal:** GitHub 스타 기준 인기 MCP 서버 + Plugin을 추가하여 DB를 60-65개로 확장한다.

**Target features:**
- 인기 MCP 서버 5-10개 추가 (GitHub README 기반 완전 검증)
- 인기 Plugin 5-10개 추가 (GitHub README 기반 완전 검증)
- 한/영 번역 동기화
- 총 DB 51개 → 60-65개
...
- 42개 MCP 서버 + 9개 Plugin = 51개 항목 DB
