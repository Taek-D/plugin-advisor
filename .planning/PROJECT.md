...
Claude Code 플러그인 추천 + 조합 분석 웹 서비스. 프로젝트 분석(/advisor)으로 맞춤 플러그인 추천, 조합 분석(/optimizer)으로 현재 설치된 MCP 서버/Plugin 점수화 + 타입별 보완/대체 추천. 51개 검증 DB, 동적 OG 소셜 카드, Umami Cloud analytics, SNS 공유/피드백/뉴스레터 인프라 포함. 한/영 다국어 지원.
...
- ✓ Umami Cloud analytics 전 페이지 자동 추적 + 광고차단기 우회 프록시 — v1.4
- ✓ 페이지별 동적 OG 이미지 (next/og Edge Runtime, 51 plugins + 6 guides) — v1.4
- ✓ SNS 공유 버튼 (Web Share API + 클립보드 + X/LinkedIn 소셜 링크) — v1.4
- ✓ 피드백 위젯 (글로벌 플로팅 버튼 + 별점 + Supabase + 관리자 페이지) — v1.4
- ✓ 뉴스레터 구독 폼 (이메일 수집 + Supabase upsert + rate limit) — v1.4
...
- 16,800+ LOC TypeScript (Next.js 14 + shadcn/ui)
...
- v1.4: Umami Cloud analytics, 동적 OG 이미지, SNS 공유/피드백/뉴스레터 인프라
...
- OG 이미지: `lib/og-utils.ts` (공유 유틸) + 각 route의 opengraph-image.tsx/twitter-image.tsx
- Supabase: `lib/supabase-types.ts` (Database 타입) + `lib/supabase-admin.ts` (서버 클라이언트)
...
| Umami Cloud (GA4/Vercel Analytics 대신) | 100K 무료 이벤트, 쿠키리스, 광고차단 우회 가능 | ✓ Good — 개발자 대상 50-70% 데이터 손실 방지 |
| zero new npm packages (v1.4 전체) | Umami 외부 스크립트, next/og 빌트인, Web Share API 네이티브 | ✓ Good — 번들 크기 무증가 |
| next.config.mjs rewrite 기반 프록시 | API route 대신 rewrite로 Umami 프록시 구현 | ✓ Good — 서버리스 함수 비용 제로 |
| OG 이미지 dark bg + Space Grotesk Bold | 브랜드 일관성 + Edge Runtime 호환 영문 폰트 | ✓ Good — 소셜 카드 시각적 일관성 |
| Guide OG는 titleEn/summaryEn 사용 | Edge Runtime 한국어 폰트 렌더링 불일치 방지 | ✓ Good — 안정적 렌더링 |
| FeedbackWidget 별점 optional (nullable) | 진입 장벽 최소화, 텍스트만으로도 제출 가능 | ✓ Good — 피드백 제출률 극대화 |
| Supabase 수동 타입 정의 (codegen 대신) | CI 없이도 타입 안전, @ts-expect-error 제거 | ✓ Good — 빌드 타임 에러 제거 |

---
*Last updated: 2026-03-30 after v1.4 milestone complete*
