...
- [ ] Plugin 타입 시스템: DB에 type 'mcp' | 'plugin' 필드 추가
- [ ] Plugin DB 10-15개 구축 (omc, fireauto, superpowers, document-skills 등)
- [ ] /plugins 페이지에서 MCP | Plugin 탭 분리
- [ ] /optimizer에서 MCP + Plugin 통합 분석 (동일 감점 모델)
- [ ] /optimizer 파서: claude plugin list 포맷 완전 지원
- [ ] i18n 업데이트 (탭, 타입 관련 번역)
...
## Current Milestone: v1.2 MCP + Plugin 통합

**Goal:** MCP 서버와 Plugin을 구분하여 관리하고, /optimizer에서 통합 분석할 수 있게 한다.

**Target features:**
- DB 타입 시스템 (type: 'mcp' | 'plugin')
- 주요 Plugin 10-15개 DB 구축
- /plugins MCP | Plugin 탭 분리
- /optimizer MCP + Plugin 통합 점수화
